...
### Legrand Ecocompteur Adapter Context

This adapter interfaces with the Legrand Ecocompteur module (aka. Legrand Measurement Concentrator EMDX³ 412000), an energy monitoring device that provides real-time electrical consumption data for multiple circuits.

**Key Device Characteristics:**
- **Device Model**: Legrand EMDX³ 412000 measurement concentrator  
- **Communication**: HTTP-based API (JSON and HTML endpoints)
- **Data Types**: Instantaneous power readings, index values (TIC protocol), circuit labels
- **Polling Strategy**: Dual-interval system (JSON for frequent updates, Index for cumulative data)
- **Network Fragility**: Device has a fragile IP stack requiring careful timeout handling and proxy considerations

**Unique Requirements:**
- **Dual Polling System**: JSON endpoint for fast power readings, Index HTML parsing for TIC data
- **Reading Validation**: Configurable maximum power validation to filter out spurious readings
- **Timeout Management**: Critical handling of network timeouts due to device limitations  
- **Proxy Support**: May require nginx proxy for network stability (documented in README)
- **Resource Management**: Single interval timer pattern to minimize device load
...
### Legrand Ecocompteur Specific Patterns

**HTTP Request Handling:**
- Always use axios with explicit timeout configuration
- Implement retry logic for network failures due to device fragility
- Use promise-based patterns with proper error handling for timeout scenarios

**Polling Implementation:**
- Use `set-interval-async` for non-blocking interval management
- Implement dual-polling pattern: prioritize most overdue endpoint (JSON vs Index)
- Single timer approach: `Math.min(pollJSON, pollIndex)` for optimal efficiency
- Track `this.lastJSON` and `this.lastIndex` timestamps for scheduling decisions

**Data Processing:**
- Parse JSON responses for instantaneous power readings
- HTML parsing for TIC index data from the device's web interface  
- Implement reading validation using configurable maximum thresholds
- Handle missing or null circuit data gracefully (not all circuits may be active)

**State Management:**
- Circuit data structure: `circuits[n].{name, powerStateName, labelStateName}`
- Create channels for organized state hierarchy
- Implement proper kWh calculations from index differences
- Maintain state metadata for cumulative energy calculations

**Error Handling Patterns:**
- Network timeout handling is critical due to device limitations
- Implement graceful degradation when device is unreachable
- Log appropriate levels: debug for polling, warn for timeouts, error for config issues
- Never let HTTP failures crash the adapter - always catch and continue
